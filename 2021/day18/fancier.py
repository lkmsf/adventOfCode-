...
def explode(vals, level = 0, left = None, right = None):
    ...
    if level >= 4 and singlePair(vals):
        ...
        if explode(vals[0], level + 1, left, vals[1]): return True
        if explode(vals[1], level + 1, vals[0], right): return True
    return False

def reduce(vals):
    while True:
        if explode(vals): continue
        if split(vals): continue
        break
